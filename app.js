  ...
  try {
    ...
    const users = await prisma.user.findMany({
      where: {
        applicationStatus: "APPROVED",
        shifts: {
          some: {}, // Only get users who have at least one shift
        },
      },
      include: {
        shifts: {
    ...
    const todayLondon = currentTimeLondon.format("YYYY-MM-DD");
    const currentLTimeAndDate = currentTimeLondon.format("YYYY-MM-DD HH:mm:ss");
    const currentLTime = currentTimeLondon.format("HH:mm:ss");
    const twoDaysBefore = currentTimeLondon
      .clone()
      .subtract(1, "days")
      .format("YYYY-MM-DD");

    for (const user of users) {
      // Generate a unique identifier based on user ID and date
      const uniqueIp = generateUniqueId(user.id, twoDaysBefore);

      // Check if an attendance record exists for this user, date, and IP
      const attendance = await prisma.attendance.findFirst({
        where: {
          userId: user.id,
          date: twoDaysBefore,
      ...
      // Filter schedules for twoDaysBefore
      user.shifts.forEach((shift) => {
        shift.schedule.forEach((schedule) => {
          if (schedule.startTime !== null && schedule.status === true) {
            if (schedule.shiftDate === twoDaysBefore) {
              scheduleForToday.push(schedule);
            }
          }
        });
      ...
      // Proceed only if there is at least one schedule
      if (scheduleForToday.length > 0) {
        if (attendance) {
          // Update attendance if check-out is missing and check-in exists
          if (attendance.outTime === null && attendance.inTime) {
            // Extract the date part from the schedule's endTime
            const datePart = moment(scheduleForToday[0].endTime).format("YYYY-MM-DD");
            const attendanceInTimeWithDate = `${datePart} ${attendance.inTime}`;

            // Parse times using moment
            const currentOutTime = moment(
              `${datePart} ${moment(scheduleForToday[0].endTime).format("HH:mm:ss")}`,
              "YYYY-MM-DD HH:mm:ss"
            );
            ...
            // Validate parsed dates
            if (!currentOutTime.isValid() || !attendanceInTime.isValid()) {
              console.error(`Invalid date or time format for user ${user.id}`);
              continue;
            }
            ...
            await prisma.attendance.update({
              ...
              data: {
                ...
                totalHour: totalHourCombine,
                overtime: null,
                outTimeStatus: "Check out by system",
              },
            });
          }
        } else {
          // If no attendance record exists, check again using the same IP filter before creating a new one
          const attendanceRecehck = await prisma.attendance.findFirst({
            where: {
              userId: user.id,
              date: twoDaysBefore,
          ...
          if (!attendanceRecehck) {
            ...
            await prisma.attendance.create({
              data: {
                ...
                totalHour: null,
                overtime: null,
                createdAt: new Date(`${twoDaysBefore}T00:00:00Z`), // Start of day UTC
                updatedAt: new Date(`${twoDaysBefore}T00:00:00Z`),
              },
            });
          }
